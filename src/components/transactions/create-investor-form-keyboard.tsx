...
type CreateInvestorInput = z.infer<typeof createInvestorSchema>;

interface CreateInvestorFormKeyboardProps {
  onSubmit: (data: CreateInvestorInput) => void;
  isPending?: boolean;
  error?: string | null;
  success?: boolean;
  onReset?: () => void;
...
export function CreateInvestorFormKeyboard({ 
  onSubmit, 
  isPending, 
  error, 
  success, 
  onReset,
  onCancel
}: CreateInvestorFormKeyboardProps) {
  ...
  const investmentTypeInputRef = useRef<HTMLInputElement>(null);
   ...
   * 1. Sequence: Name -> Investment Type -> Email -> Phone -> Address -> Notes -> Submit
   * 2. Form Navigation:
  ...
  const formFields = [
    ...
    { ref: investmentTypeInputRef, name: 'investmentType' },
  ...
  const handleSubmit = (data: CreateInvestorInput) => {
    onSubmit(data);
  };
    ...
    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="investmentType"
            placeholder="e.g., Equity Partner, Loan Provider, Silent Partner"
            {...form.register('investmentType')}
            ref={investmentTypeInputRef}
            className="focus:ring-2 focus:ring-primary"
          />
        ...
        <Card className="bg-muted/30">
          <CardContent className="p-4">
            <div className="text-sm">
              <span className="font-medium">New Investor:</span> {form.watch('name')}
              {form.watch('investmentType') && ` | ${form.watch('investmentType')}`}
      ...
      <TransactionFormFooter
        ...
        saveLabel="Create Investor [Ctrl+Enter]"
      />
    </form>
  );
}
