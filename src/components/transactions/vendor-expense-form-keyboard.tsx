...
// Form schema
const vendorExpenseSchema = z.object({
...
type VendorExpenseInput = z.infer<typeof vendorExpenseSchema>;

interface VendorExpenseFormKeyboardProps {
  onSubmit: (data: VendorExpenseInput) => void;
  isPending?: boolean;
  error?: string | null;
  success?: boolean;
  onReset?: () => void;
...
export function VendorExpenseFormKeyboard({ 
  onSubmit, 
  isPending, 
  error, 
  success, 
  ...
  sitesLoading = false
}: VendorExpenseFormKeyboardProps) {
  ...
  const form = useForm<VendorExpenseInput>({
    resolver: zodResolver(vendorExpenseSchema),
    defaultValues: {
      amount: 0,
   ...
   * 1. Sequence: Vendor -> Site -> Description -> Amount -> Date -> Note -> Submit
   * 2. Form Navigation:
  ...
  useEffect(() => {
    const handleFormNavigation = (e: KeyboardEvent) => {
      ...
      if (vendorDropdownOpen || siteDropdownOpen) {
  ...
  }, [vendorDropdownOpen, siteDropdownOpen, formFields]);
  ...
  useEffect(() => {
    const interceptNavigationKeys = (e: KeyboardEvent) => {
      ...
      if (isAltBackspace || (isEscape && !vendorDropdownOpen && !siteDropdownOpen)) {
        e.preventDefault();
        e.stopPropagation();
        onCancel?.();
      }
  ...
  }, [onCancel, vendorDropdownOpen, siteDropdownOpen]);
  ...
  const handleSubmit = (data: VendorExpenseInput) => {
    onSubmit(data);
  };
    ...
    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
      ...
      {success && (
        <Alert className="border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/20">
          ...
          <AlertDescription className="text-green-800 dark:text-green-200">
            Vendor expense recorded successfully!
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="description"
            placeholder="What was this vendor expense for?"
