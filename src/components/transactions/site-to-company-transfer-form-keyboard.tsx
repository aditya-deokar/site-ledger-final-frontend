...
// Form schema
const siteToCompanyTransferSchema = z.object({
...
type SiteToCompanyTransferInput = z.infer<typeof siteToCompanyTransferSchema>;

interface SiteToCompanyTransferFormKeyboardProps {
  onSubmit: (data: SiteToCompanyTransferInput) => void;
  isPending?: boolean;
  error?: string | null;
  success?: boolean;
  onReset?: () => void;
...
export function SiteToCompanyTransferFormKeyboard({ 
  onSubmit, 
  isPending, 
  error, 
  success, 
  ...
  sites = [],
  sitesLoading = false,
  companyAccount = { balance: 0 }
}: SiteToCompanyTransferFormKeyboardProps) {
  ...
  const form = useForm<SiteToCompanyTransferInput>({
    resolver: zodResolver(siteToCompanyTransferSchema),
    defaultValues: {
      amount: 0,
   ...
   * 1. Sequence: Site -> Amount -> Date -> Note -> Submit
   * 2. Form Navigation:
  ...
  const handleSubmit = (data: SiteToCompanyTransferInput) => {
    onSubmit(data);
  };
    ...
    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
      ...
      {success && (
        <Alert className="border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/20">
          ...
          <AlertDescription className="text-green-800 dark:text-green-200">
            Transfer to office completed successfully!
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  filteredSites.map((site, index) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-muted-foreground">
                        ...
                        {site.balance && ` | Balance: Rs. ${site.balance.toLocaleString('en-IN')}`}
                      </div>
                    </div>
                  ))
                ) : (
        ...
        <div className="space-y-2">
          ...
          />
          {selectedSite && selectedSite.balance && (
            <p className="text-sm text-muted-foreground">
              Available: Rs. {selectedSite.balance.toLocaleString('en-IN')}
            </p>
          )}
      ...
      {/* Transfer Summary */}
      {(selectedSite || form.watch('amount') > 0) && (
        <Card className="bg-muted/30">
          <CardContent className="p-4">
            <div className="text-sm">
              <span className="font-medium">Transfer:</span> Rs. {form.watch('amount').toLocaleString('en-IN')}
              {selectedSite && ` from ${selectedSite.name}`}
              {selectedSite && selectedSite.flatNumber && ` (Flat ${selectedSite.flatNumber})`}
              {selectedSite && selectedSite.balance && (
                <div className="text-muted-foreground mt-1">
                  Remaining Site Balance: Rs. {(selectedSite.balance - form.watch('amount')).toLocaleString('en-IN')}
                </div>
              )}
              {companyAccount && (
                <div className="text-muted-foreground mt-1">
                  New Company Balance: Rs. {(companyAccount.balance + form.watch('amount')).toLocaleString('en-IN')}
                </div>
              )}
            </div>
          </CardContent>
