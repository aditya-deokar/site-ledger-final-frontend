...
// Form schema
const companyToSiteTransferSchema = z.object({
...
type CompanyToSiteTransferInput = z.infer<typeof companyToSiteTransferSchema>;

interface CompanyToSiteTransferFormKeyboardProps {
  onSubmit: (data: CompanyToSiteTransferInput) => void;
  isPending?: boolean;
  error?: string | null;
  success?: boolean;
  onReset?: () => void;
...
export function CompanyToSiteTransferFormKeyboard({ 
  onSubmit, 
  isPending, 
  error, 
  success, 
...
}: CompanyToSiteTransferFormKeyboardProps) {
  ...
  const form = useForm<CompanyToSiteTransferInput>({
    resolver: zodResolver(companyToSiteTransferSchema),
    defaultValues: {
      amount: 0,
  ...
  const handleSubmit = (data: CompanyToSiteTransferInput) => {
    onSubmit(data);
  };
      ...
      {success && (
        <Alert className="border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/20">
          ...
          <AlertDescription className="text-green-800 dark:text-green-200">
            Transfer to site completed successfully!
          </AlertDescription>
        </Alert>
      )}
        ...
        <Card className="bg-muted/30">
          <CardContent className="p-4">
            <div className="text-sm">
              <span className="font-medium">Company Balance:</span> Rs. {companyAccount.balance.toLocaleString('en-IN')}
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <div className="space-y-2">
          ...
          {companyAccount.balance > 0 && (
            <p className="text-sm text-muted-foreground">
              Available: Rs. {companyAccount.balance.toLocaleString('en-IN')}
            </p>
          )}
        ...
        <Card className="bg-muted/30">
          <CardContent className="p-4">
            <div className="text-sm">
              ...
              {selectedSite && ` to ${selectedSite.name}`}
              {selectedSite && selectedSite.flatNumber && ` (Flat ${selectedSite.flatNumber})`}
              {companyAccount.balance > 0 && (
                <div className="text-muted-foreground mt-1">
                  Remaining Company Balance: Rs. {(companyAccount.balance - form.watch('amount')).toLocaleString('en-IN')}
                </div>
              )}
            </div>
          </CardContent>
