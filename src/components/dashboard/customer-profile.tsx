...
// ── Cancel Confirm ──────────────────────────────────
function CancelConfirm({ customer, siteId, onClose, onDone }: { customer: Customer; siteId: string; onClose: () => void; onDone: () => void }) {
  const { mutate, isPending } = useCancelBooking({ onSuccess: onDone })
        ...
        <div className="px-8 py-6 flex-1 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-2 text-sm text-foreground">
                <CreditCard className="w-3.5 h-3.5 text-muted-foreground/40" /> {formatDate(customer.createdAt)}
              </div>
            </div>
          </div>
          ...
          {mode === "cancel" && <CancelConfirm customer={customer} siteId={siteId} onClose={() => setMode("view")} onDone={onClose} />}
        </div>
