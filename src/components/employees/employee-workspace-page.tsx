...
export function EmployeeWorkspacePage({
  ...
  const { data: employeeData, isLoading: isEmployeeLoading } = useEmployee(employeeId);
  const { data: attendanceData, isLoading: isAttendanceLoading } = useEmployeeAttendance(employeeId, periodMonth, periodYear);
  const { data: monthlyTransactionsData, isLoading: isMonthlyTransactionsLoading } = useEmployeeTransactions(employeeId, {
    startDate,
    endDate,
  });
  const { data: allTransactionsData, isLoading: isAllTransactionsLoading } = useEmployeeTransactions(employeeId);
        ...
        <div className="border-b border-border p-8 pb-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-4xl font-serif tracking-tight text-foreground italic">
                {employeeDetails?.name ?? 'Employee'}
              </h1>
              <p className="mt-2 text-sm text-muted-foreground">
                {(employeeDetails?.employeeId ?? 'Loading ID')} - {(employeeDetails?.designation ?? 'Loading designation')}
              </p>
            </div>
